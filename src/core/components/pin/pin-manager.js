...
class PinManager {
  ...
  async _walkDag ({ cid, preload = false, onCid = () => {} }) {
    const walk = (cid) => {
      return async () => {
        const result = await this.dag.get(cid, { preload })

        onCid(cid)

        if (result.value.Links) {
          queue.addAll(
            result.value.Links.map(link => {
      ...
      this.recursiveKeys()
        .map(key => {
          return async () => {
            ...
            const node = await this.dag.get(cid, '', { preload: false })
            const has = await this.pinset.hasDescendant(node, key)
